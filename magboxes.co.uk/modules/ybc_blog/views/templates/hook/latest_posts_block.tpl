{if $posts}
    ...
    <div class="block ybc-blog-posts-popular sidebar-post-type-{$sidebar_post_type} ybc_blog_skin_{$blog_skin}">
        <h4 class="title_block ">{l s='Latest posts' mod='ybc_blog'}</h4>
        <div class="block_content">
            <ul id="ybc-blog-posts-latest-list" class="ybc_popular_posts">
                {foreach from=$posts item='post'}
                    <li> 
                        <div class="ybc-blog-latest-post-wrapper">                        
                            ...
                            <div class="ybc-blog-latest-post-content">
                                ...
                                {if $allowComments || $show_views || $allow_like}
                                    <div class="ybc-blog-latest-toolbar">
                                        ...
                                        {/if}
                                        {if $allowComments}
                                            <span class="ybc-blog-latest-toolbar-comments">{$post.comments_num} {if $post.comments_num!=1}<span>
                                        ...
                                        <div class="ybc-blog-categories">
                                            ...
                                            <div class="be-categories">
                                                ...
                                                {foreach from=$post.categories item='cat'}
                                                    ...
                                                    <a href="{$cat.link}">{ucfirst($cat.title)}</a>{if $ik < $totalCat}<span class="comma">, </span>{/if}
                                                {/foreach}
                                            </div>
                                        </div>
                                    {/if}
