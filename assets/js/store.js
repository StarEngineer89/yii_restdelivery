...
jQuery(document).ready(function() {
    ...
    if($('body .section-menu').length > 0){
        $('body').addClass('menu_section');
    }
    ...
    if($(".backbuttononly").length > 0){

        var currentlink = $('.backbuttononly').attr("href");

        if(is_iframe){

            if(currentlink=="/menu-pizza4u"){
                $('.backbuttononly').attr('href',currentlink+'?iframe');
            }
            else {
                $('.backbuttononly').attr('href',currentlink+'/?iframe=true');
            }
...
function scroll(id){
    if( $('#'+id).is(':visible') ) {
        if (window.location !== window.parent.location) {
            $('html,body', window.parent.document).animate({scrollTop: $("#"+id).offset().top-100},'slow');
        } else {
            ...
            if (data.code==1){
                ...
                if ( action == "placeOrder" ){
                    ...
                    switch (data.details.payment_type)
                    {
                        ...
                        case "pyr":
                            if(is_iframe){
                                window.location.replace(sites_url+"/receipt/?id="+data.details.order_id+'&iframe=true');
                            }
                            else {
                        ...
                        case "sap":
                            ...
                            $('#hdn-new-merchant-session').val(data.details.new_merchant_session);
                            $('<a/>')
                                .fancybox({
                                    maxWidth:800,
                                    maxHeight: 450,
                                    ...
                                    autoSize : true,
                                    type : 'iframe',
                                    href : data.details.payment_link,
                                    openEffect :'elastic',
                                    closeEffect :'elastic'
                                })
                                .trigger('click');
                ...
                if ( action=="InitPlaceOrder"){
                    ...
                    if(is_iframe){
                        ...
                        if($("#is_guest_checkout").length > 0){
                            window.location.href = sites_url+data.details+"&iframe=true";
                        }
                        else{
                            window.location.href = sites_url+data.details+"/?iframe=true";
                        }
...
jQuery(document).ready(function() {
    ...
    $(".icheckbox_minimal").load("demo_test.txt");
    ...
    $(document).on('click','.collapse_addon_item',function(){
        $.fancybox.reposition();
    });
    $( document ).on( "click", ".menu-item", function() {
        ...
        params+="&category_id=" + $(this).data("category_id") + "&price=" + price + "&tp=" + tp + "&size_id=" + sizeid;
            ...
            $('.require_addon').each(function () {
                if ( $(this).val()==2 ) {
                    ...
                    if ( required_addon_selected <=0){
                        addon_err=js_lang.trans_47+" - "+required_addon_name +"";
                        /*alert(addon_err);*/
                        $(".require_addon_"+required_addon_id).after(
                            "<p class=\"req_addon_msg text-danger text-small top10\">"+addon_err+"<br/></p>").fadeIn("slow");
                    }
                }
            });
    ...
    $( document ).on( "click", ".checkout", function() {
        ...
        * */
        ...
        params+="&specialinstructions="+$("#specialinstructions").val();
        busy(true);
        $.ajax({
            type: "POST",
            url: ajax_url,
            ...
            success: function(data){
                ...
                if (data.code==1) {
                    if(is_iframe){
                        window.location.href=sites_url+"/checkout/?iframe=true";
                    }
                    else {
    ...
    $( document ).on( "click", ".place_order", function(e) {
        ...
        var params = "specialinstructions="+$("#specialinstructions").val();
        $.ajax({
            type: "POST",
            url: ajax_url,
            data: "action=specialInstrucitonsSelectionChange&currentController=store&tbl=specialInstrucitonsSelectionChange&"+params,
            dataType: 'json',
            success: function(data){
                busy(false);
                if (data.code==1) {
...
function open_fancy_box(params)
{
    ...
    // Additional centering for iframe content
    var topRatio = 0.5;
    if (window.location !== window.parent.location) {
        topRatio = ($(window.parent).scrollTop() -280 + window.parent.document.body.clientHeight / 2) / ($(window.document.body).height());
    }
    var window_height = $(window.parent).innerHeight();
    //alert(window_height);
    ...
    $.fancybox({
        ...
        topRatio : window_height <= 700 ? 0 : 0.05,
        padding :0,
        margin :0,
        modal:false,
        type : 'ajax',
        ...
        afterClose: function(){
            $("body",window.parent.document).css({'overflow-y':'visible'});
            $("body").css({'overflow-y':'visible'});
        },
        afterShow : function(data){
            if (window.location !== window.parent.location) {
                var height = window_height <= 700 ? window_height : 700;
                var top_value = $(window.parent).scrollTop();
                $(".fancybox-wrap").css('min-height',height+'px');
                $(".fancybox-wrap").css('max-height',height+'px');
                $(".fancybox-wrap").css('height',height+'px !important');
                $('.fancybox-wrap').css('top', top_value+'px !important');
                var offset = $(".fancybox-wrap").offset().top + $("iframe", window.parent.document).offset().top ;
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (data.code==1){
                ...
                if ($('.Pay_By_Card').prop('checked')) {
                    $('.voucher_wrap').show();
                } else  {
                    $('.voucher_wrap').hide();
                }

                $('.footer_button_cart').remove();
                var num_item = 0;
                $.each(data.details.raw.item,function(idx,val){
                    num_item = num_item+parseInt(val.qty);
                });
                ...
                $('body').addClass('has-footer_cart-button');
                $('body').prepend(button_cart);
                clearCartButton(1);
...
jQuery(document).ready(function() {
    ...
    $('.payment_sap').on('ifChecked', function(event){
        $("#change_wrap").slideToggle("fast");
    });

    $('.payment_sap').on('ifUnchecked', function(event){
        $("#change_wrap").slideToggle("fast");
    });
