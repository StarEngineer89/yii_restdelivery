...
jQuery(document).ready(function() {
  ...
  $(document ).on( "click", ".row_duplicate", function() {
    
        row_duplicate( $(this).attr("rev"),$("#tbl").val(), $(this));
    });
...
function row_duplicate(id,tbl,object){
  rm_notices();
  var form_id=$("form").attr("id");
  busy(true);
  
  var params="action=getFullAddonCategoryList&yii_session_token="+yii_session_token; 
  $.ajax({    
        type: "POST",
        url: ajax_url,
        data: params,
        ...
        success: function(data){
          busy(false);
          if (data.code==1){
            var html = '<div class="uk-form"><div class="uk-form-row"><select class="uk-form-width-large" id="duplicate_select_category">'
            $.each(data.msg,function(idx,val){
              html += '<option value="'+val.id+'">'+val.name+'</option>';
            });
            html += '</select></div></div>';
            $.UIkit.modal.confirm(html,function(){
              busy(true);
              var select_id = $('#duplicate_select_category').val();
              var duplicate_params = 'action=onDuplicateCategory&id='+id+'&to='+select_id+'&yii_session_token='+yii_session_token;
              $.ajax({
                url: ajax_url,
                data:duplicate_params,
                type: 'POST',
                dataType: 'json',
                success: function(response){
                  busy(false);
                  if (data.code==1){
                    $("#"+form_id).before("<div class=\"uk-alert uk-alert-success\">Duplicated</div>");  
                  }
                  else {
                    $("#"+form_id).before("<div class=\"uk-alert uk-alert-danger\">ERROR:</div>");
                  }
...
jQuery(document).ready(function() {	
  ...
  $('.show_exclude_item').click(function(){
    var id = $(this).attr('data-id');
    if($(this).hasClass('exclude_open')){
      $(this).removeClass('exclude_open');
      $('#exclude_item_'+id).hide();
    }
    else {
      $('.list_exclude_item').hide();
      $(this).addClass('exclude_open');
      $('#exclude_item_'+id).show();
    }
  });
  $(document).on('click','.edit_item_price',function(){
    var item_id = $(this).attr('data-id');
    var position = $(this).attr('data-key');
    $('.block_edit_item_price').hide();
    $(this).parent().hide();
    $('#block_edit_item_price_'+item_id+'_'+position).show();
  });
  $(document).on('click','.save_edit_item_price',function(){
    var currency = $(this).attr('data-currency');
    var sizec = $(this).attr('data-size');
    var item_id = $(this).attr('data-id');
    var position = $(this).attr('data-key');
    $('#block_edit_item_price_'+item_id+'_'+position).hide();
    $('#block_showing_price_'+item_id+'_'+position).show();
    var new_price = $(this).parent().find('.item_input_price').val();
    $('#block_showing_price_'+item_id+'_'+position).find('.item_ajax_price').html(new_price);
    $.ajax({
      url: ajax_url,
      dataType: 'json',
      data: {action:'editPriceFoodItem',id:item_id,key:position,price:new_price},
      type: 'POST',
      success: function(response){
  ...
  $('#filtercategory').on('change',function(){
    ...
    $.ajax({
      url: ajax_url,
      dataType: 'json',
      data: {action:'filterCategoryItem',id:id},
      type: 'POST',
      success: function(response){
        $('#frm_table_list').submit();
      }
    });
  });
}); /*END DOCU*/
