...
jQuery(document).ready(function() {
    ...
    $( document ).on( "click", ".filter-box a", function() {
        ...
        var t=$(this);
        var i=t.find("i");
        if ( i.hasClass("ion-ios-chevron-down")){
            i.removeClass("ion-ios-chevron-down");
            i.addClass("ion-ios-chevron-up");
        } else {
            i.addClass("ion-ios-chevron-down");
            i.removeClass("ion-ios-chevron-up");
        }
    ...
    $( document ).on( "click", ".menu-1 .menu-cat a", function() {
        ...
        if ( i.hasClass("ion-ios-arrow-down")){
            i.removeClass("ion-ios-arrow-down");

            i.addClass("ion-ios-arrow-up");
        } else {
            i.addClass("ion-ios-arrow-down");

            i.removeClass("ion-ios-arrow-up");
        }
    ...
    if ( $(".section-address-book").exists() ){
        if ( $("#table_list_info").exists() === true){
            console.log();
        } else {
            table();
        }
...
$(document).ready(function () {
    ...
    $('.Pay_By_Card').on( 'ifChecked', function () {
        $('.voucher_wrap').show();
    });

    $('.Pay_By_Cash').on("ifChecked", function () {
        remove_voucher();
    });

    $.each($('.countdown-timer'), function (i, ele) {
        var time = $(this).attr('data-time');
        countDownTimer(time, $(this));
    });

    var search = window.location.search;
    $('.cart-mobile-handle a').attr('href', $('.cart-mobile-handle a').attr('href') + search);
});

function countDownTimer(countDownTime, ele) {
    ...
    var x = setInterval(function() {
        ...
        var now = new Date();
        var nowString = now.toUTCString().slice(0, 25);
        now = new Date(nowString).getTime();
        ...
        if (distance < 0) {
            clearInterval(x);
            ele.html("Closed");
            ele.parent().parent().find('span.label').removeClass('label-success').addClass('label-danger').html('Closed');
        }
    }, 1000);
}