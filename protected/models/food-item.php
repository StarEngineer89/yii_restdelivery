  ...
  ?>
    ...
    <div class="panel-heading" role="tab" id="heading_<?php echo $val['subcat_id'];?>">
        <h4 class="panel-title">
          <a role="button"<?php if(count($data['addon_item']) > 1){echo ' data-toggle="collapse"';}?> data-parent="#accordion" class="collapse_addon_item" href="#collapse_<?php echo $val['subcat_id'];?>" aria-expanded="<?php if($tuyen==1){echo 'true';}?>" aria-controls="collapse_<?php echo $val['subcat_id'];?>"><?php echo qTranslate($val['subcat_name'],'subcat_name',$val)?><?php if(count($data['addon_item']) > 1){echo ' <small style="font-size:13px;">(Click to view)</small>';}?>
              <?php
    ...
    <div class="panel-body" style="padding:0px 10px;">
	  ...
	  ?>	    
	    ...
	    <div><?php echo "<p style='display:none' class=".$val['subcat_id'].">".qTranslate($val_addon['item_description'],'item_description',$val_addon)."</p>"; ?></div>
	    <?php $x++;?>
	  <?php endforeach;?>	  
	  <?php endif;?>
    </div>
