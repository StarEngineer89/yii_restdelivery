...
class StoreController extends CController
{
    ...
    public function actionMenu()
    {
        ...
        if (is_array($res) && count($res) >= 1) {
            ...
            if ($res['status'] == "active" && $res['is_ready'] == 2) {
                ...
                if (FunctionsV3::getMerchantOpenTime($merchant_id)) {
                    $data['service_option'] = 'delivery';
                } elseif (FunctionsV3::getMerchantPickupTime($merchant_id)) {
                    $data['service_option'] = 'pickup';
                } else {
                    $data['service_option'] = '';
                }
                ...
                $this->render('menu', array(
                    ...
                    'service_option'             => $data['option-type']
                ));
    ...
    public function actionCheckout()
    {
        if (Yii::app()->functions->isClientLogin()) {
            if(!isset($_GET['iframe'])) {
                ...
                $this->redirect(Yii::app()->createUrl('/store/paymentoption?iframe=true'));
            }
            die();
        }
        ...
        if(!isset($_GET['iframe'])){
            ...
            $_SESSION['google_http_refferer'] = websiteUrl() . "/store/paymentOption?iframe=true";
        }
    ...
    public function actionSetlanguage()
    {
        ...
        if (isset($_GET['lang'])) {
            if (!empty($referrer)) {
                ...
                $redirect = Yii::app()->createUrl('store/index', array(
                    'lang' => $_GET['lang']
                ));
            }
        } else {
            if (!empty($referrer)) {
                ...
                $redirect = Yii::app()->createUrl('store/index');
            }
        }
    ...
    public function actionApicheckout()
    {
        ...
        if (isset($data['token'])) {
            ...
            if ($res = $ApiFunctions->getCart($data['token'])) {
                ...
                $_SESSION['kr_specialinstructions']['specialinstructions']=$data['specialinstructions'];
    ...
    public function actionSapInit()
    {
        $order_id = $_GET['id'];
        $order_info = Yii::app()->functions->getOrderInfo($order_id);
        $client_info = Yii::app()->functions->getClientInfo($order_info['client_id']);

        $sagePayToken = FunctionsV3::sagePayToken();
        $merchantSessionKey = $_SESSION['merchant-session'];
        $cardIdentifier = $_SESSION['card-identifier'];
        $merchantSlug = preg_replace('/[^a-zA-Z0-9]+/', '', strtoupper($_SESSION['kr_merchant_slug']));

        /* transact using merchant key and card identifier */
        $transact = array(
            ...
            'vendorTxCode'      => 'YUMMY_' . $merchantSlug . '_' . $order_id,
            'amount'            => (int)number_format($order_info['total_w_tax'] * 100,0,',',''),
            'currency'          => 'GBP',
            'description'       => $order_info['order_id_token'],
            'apply3DSecure'     => 'UseMSPSetting',
            'customerFirstName' => $client_info['first_name'],
            'customerLastName'  => $client_info['last_name'],
            'billingAddress'    => array(
                'address1'   => $order_info['street'],
                'city'       => $order_info['city'],
                'postalCode' => $order_info['state'],
                'country'    => 'GB'
            ),
            'shippingDetails'   => array(
                'recipientFirstName' => $client_info['first_name'],
                'recipientLastName'  => $client_info['last_name'],
                'shippingAddress1'   => $order_info['street'],
                'shippingCity'       => $order_info['city'],
                'shippingPostalCode' => $order_info['state'],
                'shippingCountry'    => 'GB'
            ),
            'entryMethod'       => 'Ecommerce'
        );
        if ($_SESSION['billing-different'] == '1') {
            $transact['billingAddress']['address1'] = trim($_SESSION['billing-address-1'] . ' ' . $_SESSION['billing-address-2']);
            $transact['billingAddress']['city'] = $_SESSION['billing-city'];
            $transact['billingAddress']['postalCode'] = $_SESSION['billing-postcode'];
        }
        $transact = json_encode($transact);
        $request = FunctionsV3::sagePayRequest('https://pi-' . $sagePayToken['mode'] . '.sagepay.com/api/v1/transactions', $transact, 'Basic ' . $sagePayToken['basicAuth']);

        if (isset($request['body']) && $request['body']['statusCode'] == '0000') {
            $this->sapConfirm($order_id);
        } elseif (isset($request['body']) && $request['body']['statusCode'] == '2007') {
            $this->renderPartial('sagepay-3dsecure', array(
                'acsUrl'        => $request['body']['acsUrl'],
                'termUrl'       => Yii::app()->createAbsoluteUrl('/sap3dsecure'),
                'paReq'         => $request['body']['paReq'],
                'orderId'       => $order_id,
                'transactionId' => $request['body']['transactionId']
            ));
        } else {
            $this->renderPartial('sagepay-error', array(
                'body' => $request['body']
            ));
        }
    }

    public function actionSap3dSecure()
    {
        $sagePayToken = FunctionsV3::sagePayToken();
        $md = explode('{{&}}', $_POST['MD']);
        $transactionId = $md[0];
        $orderId = $md[1];
        $paRes = json_encode(array('paRes' => $_POST['PaRes']));
        $request = FunctionsV3::sagePayRequest('https://pi-' . $sagePayToken['mode'] . '.sagepay.com/api/v1/transactions/' . $transactionId . '/3d-secure', $paRes, 'Basic ' . $sagePayToken['basicAuth']);

        if (isset($request['body']) && $request['body']['status'] == 'Authenticated') {
            $request = FunctionsV3::sagePayRequest('https://pi-' . $sagePayToken['mode'] . '.sagepay.com/api/v1/transactions/' . $transactionId, '', 'Basic ' . $sagePayToken['basicAuth']);
            if (isset($request['body']) && $request['body']['statusCode'] == '0000') {
                $this->sapConfirm($orderId);
            } else {
                $this->renderPartial('sagepay-error', array(
                    'body' => $request['body']
                ));
    ...
    private function sapConfirm($order_id)
    {
        $db_ext = new DbExt;
        $params_logs = array(
            'order_id'          => $order_id,
            'payment_type'      => 'sap',
            'raw_response'      => json_encode(array('card-identifier' => $_SESSION['card-identifier'])),
            'date_created'      => FunctionsV3::dateNow(),
            'ip_address'        => $_SERVER['REMOTE_ADDR'],
            'payment_reference' => ''
        );
        ...
        $db_ext->updateData("{{order}}", $params_update, 'order_id', $order_id);
        echo '<script>window.top.location.href = "' . Yii::app()->createAbsoluteUrl("/store/receipt/id/" . $order_id) . '";</script>';
        die();
    }
            ...
            if ($data['hidden'] == $_GET['xid']) {
                if (isset($data['razorpay_payment_id'])) {
                    if (!empty($data['razorpay_payment_id'])) {
                        ...
                        $error = t("Invalid razorpay response");
                    }
                } else {
                    $error = t("Invalid razorpay response");
                }
            } else {
                $error = t("Order id did not match");
            }
        } else {
        ...
        if (isset($data['token']) && isset($data['razorpay_payment_id'])) {
            if (!empty($data['razorpay_payment_id'])) {
                if ($res = Yii::app()->functions->getMerchantByToken($data['token'])) {
                    ...
                    $error = t("Transaction token not found");
                }
            } else {
                $error = t("Invalid razorpay response");
            }
        ...
        if (isset($data['id']) && isset($data['token'])) {
            ...
            if ($res = $db_ext->rst($stmt)) {
                ...
                if ($res[0]['date_modified'] == "0000-00-00 00:00:00" || $res[0]['date_modified'] == "") {
                    ...
                    $msg = t("Order is already accepted");
                }
            } else {
                $msg = t("Order details not found");
            }
        } else {
        ...
        if (isset($data['id']) && isset($data['token'])) {
            ...
            if ($res = $db_ext->rst($stmt)) {
                if ($res[0]['date_modified'] == "0000-00-00 00:00:00" || $res[0]['date_modified'] == "") {
                    ...
                    $msg = t("Order is already decline");
                }
            } else {
                $msg = t("Order details not found");
            }
    ...
    public function actionvoguepaysuccess()
    {
        ...
        if (isset($data['transaction_id'])) {
            ...
            $error = t("Missing transaction id");
        }
