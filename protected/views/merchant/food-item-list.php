...
<select class="uk-form-width-medium valid" name="category" id="filtercategory">
    <option value="0">Filter Category</option>
    <?php
    $cat_list=Yii::app()->functions->getCategoryList(Yii::app()->functions->getMerchantID());
    foreach($cat_list as $cat){
        $electe = ($cat['cat_id'] == $_SESSION['filter_item_category'])?' selected':'';
        echo '<option'.$electe.' value="'.$cat['cat_id'].'">'.$cat['category_name'].'</option>';
    }
    ?>
</select>
</div>
