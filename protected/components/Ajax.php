...
"</a>";*/
    ...
    public function initpaymentprovider()
    {
        ...
        switch ($this->data['purchase']) {
            case "fax_package":
                ...
                if ($this->data['payment_opt']=="pyp" || $this->data['payment_opt']=="sap" || $this->data['payment_opt']=="stp"){
    ...
    public function addVoucherNew()
    {
        ...
        $params=array(
            ...
            'amount'=>$this->data['amount'],
            'voucher_over'=>$this->data['over'],
            'expiration'=>$this->data['expiration'],
            'status'=>$this->data['status'],
            'date_created'=>FunctionsV3::dateNow(),
            'ip_address'=>$_SERVER['REMOTE_ADDR'],
            'merchant_id'=>$merchant_id,
            'hide_in_front' => $this->data['hide_in_front']
        );
        ...
        if (isset($this->data['hide_in_front'])){
            $params['hide_in_front']=$this->data['hide_in_front'];
        } else 	$params['hide_in_front']='0';
    ...
    public function InitPlaceOrder()
    {
        ...
        switch ($this->data['payment_opt']) {
            case "pyp":
                ...
                break;
            case "sap";
                $params['hdn-billing-different'] = '';
                $params['hdn-billing-address-1'] = '';
                $params['hdn-billing-address-2'] = '';
                $params['hdn-billing-city'] = '';
                $params['hdn-billing-postcode'] = '';
                $params['hdn-new-merchant-session'] = '';
                $params['hdn-merchant-session'] = '';
                $params['hdn-card-identifier'] = '';
                break;
            default:
                break;
        }
        ...
        //if ($is_guest){
        //if(!$this->data['iframe']){
        // $this->details=Yii::app()->createUrl('store/confirmorder',array('isguest'=>1)) ;
        // }
        // else {
//$this->details=Yii::app()->createUrl('store/confirmorder?iframe=true',array('isguest'=>1)) ;
        // }
        // } else {
        //if(!$this->data['iframe']){
        //  $this->details=Yii::app()->createUrl('store/confirmorder') ;
        // }
        // else {
        //  $this->details=Yii::app()->createUrl('store/confirmorder?iframe=true') ;
        // }
        // }
//	}
    ...
    public function AllOrders()
    {
        ...
        $aColumns = array(
            ...
            'sub_total','taxable_total','total_w_tax','a.status','request_from','a.date_created','special_instructions'
        );

        $sWhere=''; $sOrder=''; $sLimit=''; $sStatus='';
        ...
        if(!empty($sWhere)){
            if (isset($this->data['start_date']) && isset($this->data['end_date']))	{
                if (!empty($this->data['start_date']) && !empty($this->data['end_date'])){
                    $sWhere=" AND date_created BETWEEN  '".$this->data['start_date']." 00:00:00' AND 
    		        '".$this->data['end_date']." 23:59:00'
    		   	";
                    $_SESSION['rpt_date_range']=array(
                        'start_date'=>$this->data['start_date'],
            ...
            if ( !empty($order_status_id)){
                $sWhere.= " AND status IN ($order_status_id)";
            }
            $sWhere.=" AND a.status NOT IN ('initial_order') ";
        } else {

            if (empty($this->data['start_date']) && empty($this->data['end_date'])){
                $sWhere.=" WHERE a.date_created BETWEEN  '".date("Y-m-d")." 00:00:00' AND '".date("Y-m-d")." 23:59:00'";
                //$sWhere.=" WHERE a.date_created BETWEEN  '".$this->data['start_date']." 00:00:00' AND '".$this->data['start_date']." 23:59:00'";
            } else {
                $sWhere.=" WHERE a.date_created BETWEEN  '".$this->data['start_date']." 00:00:00' AND '".$this->data['end_date']." 23:59:00'";
            }
            if (!empty($this->data['merchant_id']) && $this->data['merchant_id'] > 0 ){
                $sWhere.=" AND a.merchant_id = ".$this->data['merchant_id'];
            }
            if(!empty($this->data['stats_id']) && $this->data['stats_id']=="Confirmed"){
                $sWhere.=" AND a.status = '".$this->data['stats_id']."'";
            }if(!empty($this->data['stats_id']) && $this->data['stats_id']=="pending"){
                $sWhere.=" AND a.status = '".$this->data['stats_id']."'";
            }if(!empty($this->data['stats_id']) && $this->data['stats_id']=="cancelled"){
                $sWhere.=" AND a.status = '".$this->data['stats_id']."'";
            }if(!empty($this->data['stats_id']) && $this->data['stats_id']=="delivered"){
                $sWhere.=" AND a.status = '".$this->data['stats_id']."'";
            }else{
                $sWhere.=" AND a.status NOT IN ('initial_order')";
            }
