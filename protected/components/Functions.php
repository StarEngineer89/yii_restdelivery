...
class Functions extends CApplicationComponent
{
    ...
    public function jsLanguageAdmin()
    {
        ...
        return array(
            ...
            'trans_53'=>t("Sorry Your Total Must Be Above Voucher Over To Use This Voucher")
        );
    }
    ...
    public function getBasicAuthentication(){
        ...
        if (isset($_SERVER['PHP_AUTH_USER']) && isset($_SERVER['PHP_AUTH_PW'])){
            ...
            if ($Username == 'omair@yummytakeaways.co.uk' && $Password == 'VB.NET@2017!'){
    ...
    */
            ...
            print "You didn't put any credentials\n That's why you are not going to be login.";
            return false;
            ...
            //header( 'Location: https://www.yummytakeaways.co.uk/admin/login' ) ;
            return true;
        }
    }
    ...
    public function addOnItemToArray($json_data='',
                                     $multi_option='',$multi_option_val,$two_flavors_position='',$require_addon='',$merhant_id='',$tp=0,$itemName='',$item_item_id=0)
    {
        $data='';
        $data_sub='';
        $json_data=!empty($json_data)?json_decode($json_data):false;
        ...
        if ($merhant_id!=''&&$item_item_id!=0) {
            $stmt="
				SELECT * FROM
				{{item}}
				WHERE
				merchant_id='".$merhant_id."'
				AND 
            ...
            if (is_array($rows) && count($rows)>=1){
                foreach ($rows as $val) {
                    $test_column = json_decode($val['meal_selection'], true);
                }
            }
        }
        //dump($sub_category);
        ...
        if ( $json_data!=FALSE ){
            ...
            foreach ($json_data as $sub_id=>$val) {
                ...
                if (array_key_exists($sub_id,(array)$sub_category)){
                    $AddonType = $sub_category[$sub_id];
                    if(isset($test_column[$sub_id])){
                        //dump($test_column[$sub_id]);
                        foreach($test_column[$sub_id] as $size_fix){
                            $size_infor_fix = $this->getSize($size_fix);
                            if($size_infor_fix){
                                array_push($arr_name,$size_infor_fix['size_name']);
                            }
                        }
                    }
                    $test_column_item = $this->getSize($test_column[$sub_id][1]);

                    //echo "$itemName<br>";
                    //dump($arr_name);
                    if ($test_column[$sub_id][0]==0 || in_array($itemName,$arr_name)) {

                        foreach ($val as $subitem_id) {
                            //echo $val;
                            if ($subitem_details=$this->getAddonItem($subitem_id,'sequence')){
                                if($hasDescription == FALSE){
                                    $hasDescription = !empty($subitem_details['item_description'])?true:false;
                                }
                                $s_price = '';
                                switch ($tp) {
                                    case 1:
                                        $s_price = (!empty($subitem_details['price']) && tp === 1)?$subitem_details['price']:'';
                                        $data_sub[]=array(
                                        ...
                                        break;
                                    case 2:
                                        if (!empty($subitem_details['price2'])){
                                            $data_sub[]=array(
                                                'sub_item_id'=>$subitem_details['sub_item_id'],
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$test_column[$sub_id][1],
                                                'price'=>$subitem_details['price2'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                            );
                                            ...
                                            $data_sub[]=array(
                                                ...
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$subitem_details['sub_item_id'],
                                                'price'=>$subitem_details['price'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                        ...
                                        break;
                                    case 3:
                                        if(!empty($subitem_details['price3'])){
                                            $data_sub[]=array(
                                                'sub_item_id'=>$subitem_details['sub_item_id'],
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$subitem_details['sub_item_id'],
                                                'price'=>$subitem_details['price3'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                            );
                                        ...
                                        break;
                                    case 4:
                                        if(!empty($subitem_details['price4'])){
                                            $data_sub[]=array(
                                                'sub_item_id'=>$subitem_details['sub_item_id'],
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$subitem_details['sub_item_id'],
                                                'price'=>$subitem_details['price4'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                            );
                                        ...
                                        break;
                                    case 5:
                                        if(!empty($subitem_details['price5'])){
                                            $data_sub[]=array(
                                                'sub_item_id'=>$subitem_details['sub_item_id'],
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$subitem_details['sub_item_id'],
                                                'price'=>$subitem_details['price5'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                            );
                                        ...
                                        break;
                                    case 6:
                                        if(!empty($subitem_details['price6'])){
                                            $data_sub[]=array(
                                                'sub_item_id'=>$subitem_details['sub_item_id'],
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$subitem_details['sub_item_id'],
                                                'price'=>$subitem_details['price6'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                            );
                                    ...
                                    case 7:
                                        if(!empty($subitem_details['price7'])){
                                            $data_sub[]=array(
                                                'sub_item_id'=>$subitem_details['sub_item_id'],
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$subitem_details['sub_item_id'],
                                                'price'=>$subitem_details['price7'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                            );
                                    ...
                                    case 8:
                                        if(!empty($subitem_details['price8'])){
                                            $data_sub[]=array(
                                                'sub_item_id'=>$subitem_details['sub_item_id'],
                                                'sub_item_name'=>$subitem_details['sub_item_name'],
                                                'item_description'=>$subitem_details['item_description'],
                                                //'sizeId'=>$subitem_details['sub_item_id'],
                                                'price'=>$subitem_details['price8'],
                                                'photo'=>$subitem_details['photo'],
                                                'sub_item_name_trans'=>!empty($subitem_details['sub_item_name_trans'])?json_decode($subitem_details['sub_item_name_trans'],true):'',
                                                'item_description_trans'=>!empty($subitem_details['item_description_trans'])?json_decode($subitem_details['item_description_trans'],true):'',
                                            );
                ...
                // 		break;
                //	}
                if (strpos($sub_category[$sub_id], 'topping') !== FALSE){
                    $tp=0;
                }
            }
            //	}
            ...
            foreach ($res as $val) {
                ...
                if ($addon==TRUE){
                    $data[]=array(
                        ...
                        'option_values'=>!empty($val['option_values'])?json_decode($val['option_values'],true):''
    ...
    public function getItemById($item_id='',$addon=true,$tp=0)
    {
        $DbExt=new DbExt;
        $data='';
        $stmt="SELECT * FROM
        ...
        if ( $res=$DbExt->rst($stmt)){
            foreach ($res as $val) {
                ...
                //$addon_item=$this->getTopping($merchant_id,$tp);
                //}else{
    ...
    public function getTopping($mtid = 0,$tp = 0){
        $DbExt=new DbExt;
        $stmt="SELECT * FROM
		{{toppings}}
		WHERE
        ...
        if ( $res=$DbExt->rst($stmt)){
            ...
            foreach ($res as $val) {
                switch ($tp) {
                    case 1:
                        $data_sub[]=array(
                            'sub_item_id'=>$val['topping_id'],
                            'sub_item_name'=>$val['topping_name'],
                            'item_description'=>'',
                            'price'=>$val['small_price'],
                            'photo'=>'',
                            'sub_item_name_trans'=>'',
            ...
            $data[]=array(
                'subcat_id'=>0,
                'subcat_name'=>'Choose Toppings',
                'subcat_name_trans'=>$this->getSubcategoryTranslation(0),
                'multi_option'=>'',
                'multi_option_val'=>'',
                'two_flavor_position'=>'',
                'require_addons'=>'',
        ...
        if (isset($cart_item)){
            ...
            if (is_array($cart_item) && count($cart_item)>=1){
                ...
                foreach ($cart_item as $key=>$val) {
                    ...
                    /* These Line of code cut from below code where they are hide.*/
                    ...
                    //$list_free_items[$val['size_id']]
                    //$price=cleanNumber(unPrettyPrice($val['price']));

                    $price=cleanNumber(unPrettyPrice('£'.$list_free_items1[$val['size_id']]));
                    if($price==0){
                        $price=cleanNumber(unPrettyPrice($val['price']));
                    }
                    ...
                    if (!empty($size_words)){
                        ...
                        //$htm.=" ".$size_words."=".$size_info_trans."=".$t[1];
                    }
                    ...
                    if (isset($val['ingredients'])){
                        ...
                        if (!empty($val['ingredients'])){
                            if (is_array($val['ingredients']) && count($val['ingredients'])>=1){
                                $htm.="<p class=\"b uk-text-success\">".t("Includes").":</p>";
                                //$htm.="<p class=\"small ingredients-label\">Includes:</p>";
                    ...
                    //$htm.='<p style="display:none;">'.$val['size_id'].'</p>';
                    //$htm.='<p>'.$list_free_items[$val['size_id']].'</p>';
                    ...
                    if (is_array($val['sub_item']) && count($val['sub_item'])>=1){
                        foreach ($val['sub_item'] as $cat_id=>$val_sub) {
                            ...
                            foreach ($val_sub as $addon_row=> $val_subs) {
                                ...
                                if(isset($list_free_items[$cat_id]) && $list_free_items[$cat_id] > 0 && $list_free_items[$cat_id] != ''){
                                    if($number_addon < $list_free_items[$cat_id]){
                                        if(isset($list_exclude_items[$cat_id]) && is_array($list_exclude_items[$cat_id]) && in_array($val_subs[0],$list_exclude_items[$cat_id])){
                                            ...
                                            $number_addon++;
                                            $addon_raw_price=prettyFormat('0.00');
                                            $addon_item_price= unPrettyPrice('0.00');
                                            $addon_item_price=$addon_qty*$addon_item_price;
                                        }
                                    }
                                    else {
                                ...
                                $htm.='<div class="a">'.$addon_qty.'x</div>';
                                $htm.='<div class="b uk-text-muted">'." ".qTranslate($val_subs[2],
                                        'sub_item_name',$addon_name_trans).'</div>';
                ...
                if (isset($_SESSION['voucher_code'])){
                    if (is_array($_SESSION['voucher_code']) && ($subtotal > $_SESSION['voucher_code']['voucher_over'])){
                        $has_voucher=true;
                ...
                if ( $has_voucher==TRUE){
                    ...
                    $htm.=FunctionsV3::receiptRowTotal( Yii::t("default","Less Voucher")." ".$voucher_type . $voucher_code ,
                ...
                if ( $receipt!==TRUE){
                    $htm.="<div class=\"col-md-12 col-xs-12 \"><textarea type=\"textarea\" class=\"form-control\" name=\"specialinstrctions\" placeholder=\"Special Instructions\" id=\"specialinstructions\">".$_SESSION['kr_specialinstructions']['specialinstructions']."</textarea></div>";
                }
                else{
                    $htm.="<div class=\"col-md-12 col-xs-12 \">Special Instructions:</div>";
                    $htm.="<div class=\"col-md-12 col-xs-12 \">".$_SESSION['kr_specialinstructions']['specialinstructions']."</div>";
                }
                $htm.="</div>";
                ...
                $htm.=CHtml::hiddenField("subtotal_order",number_format(unPrettyPrice($subtotal+$less_voucher+$pts_redeem_amt),2));

                $htm.=CHtml::hiddenField("subtotal_order2",number_format(unPrettyPrice($subtotal),2));
                $htm.=CHtml::hiddenField("subtotal_extra_charge",
                    number_format(unPrettyPrice($delivery_charges+$merchant_packaging_charge+$taxable_subtotal),2));
    ...
    public function getOrderInfo($order_id='')
    {
        $stmt="SELECT * FROM
		{{order}} ord
		LEFT JOIN {{order_delivery_address}} oda ON oda.order_id = ord.order_id
...
//$menu_html.=PointsProgram::frontMenu(false);
    ...
    public function getOpenCloseShopStatus($merchantId='')
    {
        $lists='';
        $DbExt=new DbExt;
        $stmt="SELECT * FROM
        ...
        if ( $res=$DbExt->rst($stmt)){
            $_SESSION["testQuery"] = $res[0]['IsOpen'];
            return $res[0]['IsOpen'];
        }
        return false;
    }
    ...
    public function isMerchantOpen($mtid='')
    {
        ...
        if (!empty($mtid)){
            $_SESSION["testQuery2"] = $mtid;
            $isOpen=Yii::app()->functions->getOpenCloseShopStatus($mtid);
            ...
            /*
             * Updated statud by CStar
             * */
            $isDeliveryOpen = Yii::app()->functions->isMerchantOpenDelivery($mtid);
            $isPickupOpen = Yii::app()->functions->isMerchantOpenPickup($mtid);
    ...
    public function isMerchantOpenDelivery($mtid='')
    {
        //http://stackoverflow.com/questions/14904864/determine-if-business-is-open-closed-based-on-business-hours
        //http://php.net/manual/en/class.datetime.php
        ...
        if (!empty($mtid)){
            ...
            $times=$this->getDeliveryServiceHours($mtid);
            if (empty($times)){
    ...
    public function isMerchantOpenPickup($mtid='')
    {
        //http://stackoverflow.com/questions/14904864/determine-if-business-is-open-closed-based-on-business-hours
        //http://php.net/manual/en/class.datetime.php
        ...
        if (!empty($mtid)){
            ...
            $times=$this->getPickupServiceHours($mtid);
            if (empty($times)){
    ...
    public function getBusinnesHours($merchant_id='')
    {
        ...
        if (is_array($stores_open_day) && count($stores_open_day)>=1){
            foreach ($stores_open_day as $days) {
                ...
                if (!empty($start) && !empty($end)){
                    $business_hours[$days1]['start'] = $start;
                    $business_hours[$days1]['end'] = $end;
                }
                if (!empty($pm_starts) && !empty($pm_ends)){
                    $business_hours[$days1]['start_pm'] = $pm_starts;
                    $business_hours[$days1]['end_pm'] = $pm_ends;
                }
            }
        }
         ...
         * Expanded by CStar
         * */
        $stores_pickup_day=Yii::app()->functions->getOption("stores_pickup_day",$merchant_id);
        $stores_pickup_starts=Yii::app()->functions->getOption("stores_pickup_starts",$merchant_id);
        $stores_pickup_ends=Yii::app()->functions->getOption("stores_pickup_ends",$merchant_id);
        $stores_pickup_custom_text=Yii::app()->functions->getOption("stores_pickup_custom_text",$merchant_id);

        $stores_pickup_day=!empty($stores_pickup_day)?(array)json_decode($stores_pickup_day):false;
        $stores_pickup_starts=!empty($stores_pickup_starts)?(array)json_decode($stores_pickup_starts):false;
        $stores_pickup_ends=!empty($stores_pickup_ends)?(array)json_decode($stores_pickup_ends):false;


        $stores_pickup_pm_start=Yii::app()->functions->getOption("stores_pickup_pm_start",$merchant_id);
        $stores_pickup_pm_start=!empty($stores_pickup_pm_start)?(array)json_decode($stores_pickup_pm_start):false;

        $stores_pickup_pm_ends=Yii::app()->functions->getOption("stores_pickup_pm_ends",$merchant_id);
        $stores_pickup_pm_ends=!empty($stores_pickup_pm_ends)?(array)json_decode($stores_pickup_pm_ends):false;

        if (is_array($stores_pickup_day) && count($stores_pickup_day)>=1){
            foreach ($stores_pickup_day as $days) {
                $days2=substr($days,0,3);
                $start=''; $end='';
                if (array_key_exists($days,$stores_pickup_starts)){
                    if (!empty($stores_pickup_starts[$days])){
                        $start=date("h:i A",strtotime($stores_pickup_starts[$days]));
                    }

                    if ($start != '' && (!isset($business_hours[$days2]['start']) || $start < $business_hours[$days2]['start']) ) {
                        $business_hours[$days2]['start'] = $start;
                    }
                }
                if (array_key_exists($days,(array)$stores_pickup_ends)){
                    if (!empty($stores_pickup_ends[$days]))  	  {
                        $end=date("h:i A",strtotime($stores_pickup_ends[$days]));
                    }

                    if ($end != '' && (!isset($business_hours[$days2]['end']) || $end > $business_hours[$days2]['end']) ) {
                        $business_hours[$days2]['end'] = $end;
                    }
                }

                $pm_starts=''; $pm_ends='';
                if (array_key_exists($days,(array)$stores_pickup_pm_start)){
                    if (!empty($stores_pickup_pm_start[$days])){
                        $pm_starts=date("h:i A",strtotime($stores_pickup_pm_start[$days]));
                    }

                    if ($pm_starts != '' && (!isset($business_hours[$days2]['start_pm']) || $pm_starts < $business_hours[$days2]['start_pm']) ) {
                        $business_hours[$days2]['start_pm'] = $pm_starts;
                    }
                }
                if (array_key_exists($days,(array)$stores_pickup_pm_ends)){
                    if (!empty($stores_pickup_pm_ends[$days])){
                        $pm_ends=date("h:i A",strtotime($stores_pickup_pm_ends[$days]));
                    }

                    if ($pm_ends != '' && (!isset($business_hours[$days2]['end_pm']) || $pm_ends > $business_hours[$days2]['end_pm']) ) {
                        $business_hours[$days2]['end_pm'] = $pm_ends;
                    }
                }
            }
        } else {
        ...
        if (is_array($business_hours) && count($business_hours) >= 1) {
            ...
            foreach ($business_hours as $day => $hours) {
                ...
                if (isset($hours['start_pm']) && isset($hours['end_pm'])){
                    $pm_starts = $hours['start_pm'];
                    $pm_ends = $hours['end_pm'];
                    $business_hours[$day].=",$pm_starts - $pm_ends";
                }

                if ($business_hours[$day] == '' || empty($business_hours[$day])) {
                    unset($business_hours[$day]);
                }
            }
        }
        /* End expand by CStar*/
    ...
    public function getDeliveryServiceHours($merchant_id='')
    {
        $stores_open_day=Yii::app()->functions->getOption("stores_open_day",$merchant_id);
        $stores_open_starts=Yii::app()->functions->getOption("stores_open_starts",$merchant_id);
        $stores_open_ends=Yii::app()->functions->getOption("stores_open_ends",$merchant_id);
    ...
    public function getPickupServiceHours($merchant_id='')
    {
        $stores_pickup_day=Yii::app()->functions->getOption("stores_pickup_day",$merchant_id);
        $stores_pickup_starts=Yii::app()->functions->getOption("stores_pickup_starts",$merchant_id);
        $stores_pickup_ends=Yii::app()->functions->getOption("stores_pickup_ends",$merchant_id);
    ...
    public function getDeliveryTime($order_id='')
    {
        $DbExt=new DbExt;
        $stmt="
		SELECT status,trans_type,delivery_time FROM
		{{order}}
		WHERE
		order_id =".$this->q($order_id)."
		LIMIT 0,1
    ...
    public function getCommissionOrderStatsArray()
    {
        ...
        if (!empty($total_commission_status)){
            ...
            $total_commission_status=array('Confirmed');
        }
        /*$status='';
    	if (is_array($total_commission_status) && count($total_commission_status)>=1){
    		foreach ($total_commission_status as $val) {
