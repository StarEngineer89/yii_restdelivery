...
class FunctionsV3
{
    ...
    public static function getMenu($class="menu")
    {
		...
		//$signup_link="/store/merchantsignup";                                                    //fixed by asif 10/8/2017
		$signup_link="/store/merchantsignupinfo";
       ...
       $top_menu[]=array('visible'=>$client_signup,
        ...
        if ( Yii::app()->functions->isClientLogin()){
        	...
        	$top_menu[]=array(
        	  ...
        	  'label'=>t("Sign Out"),
        	  'itemOptions'=>array('class'=>'logout-menu'
        	 )
        	);
        }
    ...
    public static function searchByAddress($address='',$page=0,$per_page=5,$getdata='')
    {
		...
		* cos( radians( lontitude - $long) ) /* cos( radians( lontitude ) - radians($long) ) */
		+ sin( radians($lat) ) * sin( radians( latitude ) ) ) ) 
		AS distance,
		concat(street,' ',city,' ',state,' ',post_code) as merchant_address 
		
		FROM {{view_merchant}}  a 
		HAVING distance < 15	/* original 4 */
		$and
		$sort_combine";
		...
		if ($res=$DbExt->rst($stmt)){
            ...
            /*
             *  Filter Merchants by Open status
             *  Added by CStar
            */
            if (is_array($res)) {
                $opened = array();
                ...
                foreach ($res as $index => $item) {
                    ...
                    $isInDistance = FunctionsV3::isInDistance($item, $lat, $long);

                    if ($isInDistance && is_array($isInDistance)) {
                        $total_found++;
                        $result = FunctionsV3::merchantOpenCheck($item['merchant_id']);
                        $item['distance'] = $isInDistance['distance'];
                        $item['distance_type'] = $isInDistance['distance_type'];
                        switch ($result) {
                            case 1:
                                /*$opened[] = $item['merchant_id'];*/
                                $opened[] = $item;
                                break;
                            case 2:
                                /*$preorder[] = $item['merchant_id'];*/
                                $preorder[] = $item;
                                break;
                            case  3:
                                /*$closed[] = $item['merchant_id'];*/
                                $closed[] = $item;
                                break;
                            default:
                                /*$others[] = $item['merchant_id'];*/
                                $others[] = $item;
                                break;
                        }
                    }
                ...
                $res = array_merge($opened, $preorder, $closed, $others);
                /*$list = array_merge($opened, $preorder, $closed, $others);*/
                /*$list = implode(', ', $list);*/;

                /*$stmt_sorted="SELECT SQL_CALC_FOUND_ROWS a.*, ( $distance_exp * acos( cos( radians($lat) ) * cos( radians( latitude ) )
                    * cos( radians( lontitude ) - radians($long) ) 
                    ...
                    FROM {{view_merchant}}  a 
                    WHERE merchant_id IN ($list)
                       $and
                       
                    ORDER BY FIELD(`merchant_id`, $list) 
                    LIMIT $page,$per_page
                       ";*/
            }

            if ($total_found > 0) {
                /*$res=$DbExt->rst($stmt_sorted);*/
                /* End by CStar */
    ...
    public static function merchantOpenTag($merchant_id='')
    {
    	$is_merchant_open = Yii::app()->functions->isMerchantOpen($merchant_id);
         $_SESSION["testQuery1"] = $is_merchant_open == true ? 'true' : 'false';
    ...
    public static function merchantOpenCheck($merchant_id='')
    {
        $is_merchant_open = Yii::app()->functions->isMerchantOpen($merchant_id);
        $_SESSION["testQuery1"] = $is_merchant_open == true ? 'true' : 'false';;
        $merchant_preorder= Yii::app()->functions->getOption("merchant_preorder",$merchant_id);
    	  	 ...
    	  	 if (is_object($data)){
    	  	 	if ($data->status=="OK"){
    	  	 		...
    	  	 		if ($units_params=="imperial"){
                        ...
                        if (preg_match("/mi/i", $distance_value)) {
                            self::$distance_type_result='miles';
                        }
    	  	 		} else {
	...
	public static function getItemFirstPrice($prices='',$discount='')
	{
		...
		if (is_array($prices) && count($prices)>=1){
		 if(true){
		   ...
		   for ($x = 0; $x <= count($prices) - 1; $x++) {
    			//echo "$x <i class='ion-ios-plus-outline green-color bold'></i> <br>";

			$regular_price=$prices[$x]['price'];
			if(isset($prices[$x]['size'])){
				if(!empty($prices[$x]['size'])){
					$size=$prices[$x]['size'];
				}
			}
			...
			$result = $size." ".self::prettyPrice($regular_price);
			//echo "$result<br/>";
			array_push($arrayOfList,$result);
		     }
		     return $arrayOfList;
		     /*if(!empty($size)){
...
/*	public static function getItemFirstPrice($prices='',$discount='')
    ...
    public static function merchantActiveVoucher($merchant_id='')
    {
	    ...
	    $stmt="SELECT * FROM
			...
			AND
			now() <= expiration
			AND ( merchant_id =".self::q($merchant_id)." OR  merchant_id = 0 )
			AND (joining_merchant LIKE '%$mtid%' OR joining_merchant = '' OR joining_merchant = NULL) 
			LIMIT 0,10			
		";
	    //dump($stmt);
		if ( $res=$DbExt->rst($stmt)){
    ...
    public static function getAdminPaymentList()
    {
    	...
    	if (getOptionA('admin_enabled_sagepay')==""){
    		$payment_available[]='sap';
    	}
    ...
    public static function getMerchantPaymentList($merchant_id='')
    {
		...
		if ($merchant_type==2){
			...
			if (getOptionA('admin_enabled_sagepay')=="yes"){
    		          $payment_available[]='sap';
    	                       }
		...
		if ( getOption($merchant_id,'merchant_switch_master_cod')==2){
			//cod
			if (array_key_exists('Cash',(array)$new_payment_list)){
				unset($new_payment_list['Cash']);
			}
		}
   ...
   public static function getFeatureMerchant()
    ...
    public static function sagePayToken()
    {
        $mode = 'test';
        ...
        if($mode == 'test') {
            ...
            $vendorName = 'yummytakeaways';
            $integrationKey = 'TYoVNd3XAFQxPUi0VO5q6E5PuyIAUkRMpEUSrBlpgoC6CWg2S3';
            $integrationPassword = 'nYMovOTd6keQAaH1YPE7P0SJKfHJNy4H3EUrNJCs2AlL4SllYguhbxmJajFyr729c';
        }
        ...
        return array(
            ...
            'basicAuth' => base64_encode($integrationKey . ':' . $integrationPassword)
        );
    }

	public static function sagePayRequest($url, $post, $auth)
    {
        $curl = curl_init();
        ...
        $body = explode("\n", $response);
        $body = json_decode(trim(end($body)), true);
    ...
    public function getMerchantOpenTime($merchant_id='')
    {
        $is_merchant_open = Yii::app()->functions->isMerchantOpen($merchant_id);
        $_SESSION["testQuery1"] = $is_merchant_open == true ? 'true' : 'false';;
        $merchant_preorder= Yii::app()->functions->getOption("merchant_preorder",$merchant_id);
...
/*
        ...
        $full_booking_day=strtolower(date("D",strtotime(date('Y-m-d H:i:s'))));
        $booking_time=date('H:i',strtotime(date('Y-m-d H:i:s')));
            ...
            if (!array_key_exists($full_booking_day,$business_hours)){
                ...
                if (!empty($booking_time)){
                    if (array_key_exists($full_booking_day,$business_hours)){
                        ...
                        //dump($selected_date);
                        $t1 = date('H:i', strtotime(trim($selected_date[0])));
                        $t2 = date('H:i', strtotime(trim($selected_date[1])));

                        if (count($temp_selected)>=1 && !empty($temp_selected[1])) {
                            $selected_date2=explode("-",$temp_selected[1]);

                            $t3 = date('H:i', strtotime(trim($selected_date2[0])));
                            $t4 = date('H:i', strtotime(trim($selected_date2[1])));

                            if ( Yii::app()->functions->checkBetweenTime($booking_time,$t1,$t4)){
                                if ( Yii::app()->functions->checkBetweenTime($booking_time, $t1, $t2) == false && Yii::app()->functions->checkBetweenTime($booking_time, $t3, $t4) == false){
                            ...
                            if ( Yii::app()->functions->checkBetweenTime($booking_time,$t1,$t2) == false){
                                return array($t1, $t2);
                            }
    ...
    public function getMerchantDeliveryTime($merchant_id='')
    {
        $is_merchant_open_delivery = Yii::app()->functions->isMerchantOpenDelivery($merchant_id);
        $_SESSION["testQuery1"] = $is_merchant_open_delivery == true ? 'true' : 'false';
        $merchant_preorder= Yii::app()->functions->getOption("merchant_preorder",$merchant_id);
        ...
        if ( $m_holiday=Yii::app()->functions->getMerchantHoliday($merchant_id)){
            if (in_array($now,(array)$m_holiday)){
                $is_merchant_open_delivery=false;
                $is_holiday = true;
                return "holiday";
            }
        }

        if ( $is_merchant_open_delivery==true){
            if ( getOption($merchant_id,'merchant_close_store')=="yes"){
                $is_merchant_open_delivery=false;
                $merchant_preorder=false;
            }
        ...
        if ($is_merchant_open_delivery){
            return 1;
        } else {
            $service_hours=Yii::app()->functions->getDeliveryServiceHours($merchant_id);

            $full_pickup_day=strtolower(date("D",strtotime(date('Y-m-d H:i:s'))));
            $booking_time=date('H:i',strtotime(date('Y-m-d H:i:s')));
            if (is_array($service_hours) && count($service_hours)>=1){
                if (!array_key_exists($full_pickup_day,$service_hours)){
                    return false;
                } else {
                    if (!empty($booking_time)){
                        if (array_key_exists($full_pickup_day,$service_hours)){
                            $selected_date=$service_hours[$full_pickup_day];
                            //dump($selected_date);
                            $temp_selected=explode(",",$selected_date);
                            //dump($temp_selected);
    ...
    public function getMerchantPickupTime($merchant_id='')
    {
        $is_merchant_open_pickup = Yii::app()->functions->isMerchantOpenPickup($merchant_id);
        $_SESSION["testQuery1"] = $is_merchant_open_pickup == true ? 'true' : 'false';
        $merchant_preorder= Yii::app()->functions->getOption("merchant_preorder",$merchant_id);
        ...
        if ( $m_holiday=Yii::app()->functions->getMerchantHoliday($merchant_id)){
            if (in_array($now,(array)$m_holiday)){
                $is_merchant_open_pickup=false;
                $is_holiday = true;
                return "holiday";
            }
        }

        if ( $is_merchant_open_pickup==true){
            if ( getOption($merchant_id,'merchant_close_store')=="yes"){
                $is_merchant_open_pickup=false;
                $merchant_preorder=false;
            }
        ...
        if ($is_merchant_open_pickup){
            return 1;
        } else {
            $service_hours=Yii::app()->functions->getPickupServiceHours($merchant_id);
    ...
    public function isInDistance($item, $lat, $long) {
        /* Added function - By CStar */
        ...
        $distance=FunctionsV3::getDistanceBetweenPlot(
            $lat,$long,
            $item['latitude'],$item['lontitude'],$distance_type
        );

        $new_distance_type = FunctionsV3::$distance_type_result;

        if ($merchant_delivery_distance && $merchant_delivery_distance >= $distance) {
            return array('distance' => $distance, 'distance_type' => $new_distance_type);
        }

        if ($merchant_delivery_distance && ($distance_type == "M" || $distance_type == "K") && $new_distance_type == "ft") {
            return array('distance' => $distance, 'distance_type' => $new_distance_type);
        }

        if ($merchant_delivery_distance && $distance_type == "M" && $new_distance_type == "k") {
            return array('distance' => $distance, 'distance_type' => $new_distance_type);
        }
