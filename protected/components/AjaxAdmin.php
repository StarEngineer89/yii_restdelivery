...
if (!class_exists('AjaxAdmin'))
{
    class AjaxAdmin extends DbExt
    {
        ...
        public function addMerchant()
        {
            ...
            $params['authKey']=$this->data['authKey']; //authkey storing
        ...
        public function AddOnCategoryList()
        {
            ...
            if (is_array($rows) && count($rows)>=1){
                foreach ($rows as $val) {
    	    		...
    	    		<a href=\"javascript:;\" class=\"row_duplicate\" rev=\"$val[subcat_id]\" >".Yii::t("default","Duplicate")."</a>
    	    		</div>";
                    /*$date=date('M d,Y G:i:s',strtotime($val['date_created']));
    	    		$date=Yii::app()->functions->translateDate($date);*/
                    $date=FormatDateTime($val['date_created']);
        ...
        public function getFullAddonCategoryList(){
            $stmt="
			SELECT * FROM
			{{subcategory}}
			WHERE
            ...
            if (is_array($rows) && count($rows)>=1){
                ...
                foreach ($rows as $key=>$val) {
                    $result[$key]['id']=$val['subcat_id'];
                    $result[$key]['name']=$val['subcategory_name'];
                }
                $this->code=1;
                $this->msg = $result;
            }
            else {
                $this->msg=Yii::t("default",'ERROR. Nothing to show.');
            }
        }

        public function onDuplicateCategory(){
            $from = false;
            $to = false;
            ...
            if($to && $from && $to != $from){
                ...
                if (is_array($rows) && count($rows)>=1){
                    foreach ($rows as $subcat) {
						...
						WHERE
						merchant_id='".Yii::app()->functions->getMerchantID()."'
						AND category LIKE '%\"".$subcat['subcat_id']."\"%'
						";
                        $items=$connection->createCommand($stmt)->queryAll();
                        if (is_array($items) && count($items)>=1){
                            foreach($items as $item){
                                $params=array(
                                    'sub_item_name'=>$item['sub_item_name'],
                                    'item_description'=>$item['item_description'],
                                    'category'=>json_encode(array($to)),
                                    'price'=>$item['price'],
                                    'price2'=>$item['price2'],
                                    'price3'=>$item['price3'],
                                    'price4'=>$item['price4'],
                                ...
                                $command->insert('{{subcategory_item}}',$params);
                            }
                            $this->msg=Yii::t("default",'Duplicated');
                            $this->code = 1;
                        }
                        else {
                            $this->msg=Yii::t("default",'ERROR. Nothing to show.1');
                        }
                    }
                }
                else {
                    $this->msg=Yii::t("default",'ERROR. Nothing to show.2');
                }
            }
            else {
                $this->msg=Yii::t("default",'ERROR. Nothing to show.3');
            }
        }
        ...
        public function addAddOnCategory()
        {
            $params=array(
                ...
                'price5'=>($this->data['price5']),
                'price6'=>($this->data['price6']),
                'price7'=>($this->data['price7']),
                'price8'=>($this->data['price8']),
                'merchant_id'=>Yii::app()->functions->getMerchantID(),
                'status'=>$this->data['status']
                //'discount'=>($this->data['price'])//?json_encode($this->data['price']):""
            );
            ...
            if (isset($this->data['id']) && is_numeric($this->data['id'])){
                ...
                if ($res){
                    $stmt="
					UPDATE {{subcategory_item}}
					SET price='".$this->data['price1']."', price2='".$this->data['price2']."', price3='".$this->data['price3']."', price4='".$this->data['price4']."', price5='".$this->data['price5']."', price6='".$this->data['price6']."'
					WHERE merchant_id='".Yii::app()->functions->getMerchantID()."'
					AND category LIKE '%\"".$this->data['id']."\"%'";
                    $this->qry($stmt);
                    $this->code=1;
        ...
        public function addOnItemNew()
        {
            ...
            elseif(isset($curent_category['price1']) && $curent_category['price1'] != ''){
                $price1 = $curent_category['price1'];
            }
            if(isset($this->data['price2']) && $this->data['price2'] != ''){
                $price2 = $this->data['price2'];
            }
            elseif(isset($curent_category['price2']) && $curent_category['price2'] != ''){
                $price2 = $curent_category['price2'];
            }
            if(isset($this->data['price3']) && $this->data['price3'] != ''){
                $price3 = $this->data['price3'];
            }
            elseif(isset($curent_category['price3']) && $curent_category['price3'] != ''){
                $price3 = $curent_category['price3'];
            }
            if(isset($this->data['price4']) && $this->data['price4'] != ''){
                $price4 = $this->data['price4'];
            }
            elseif(isset($curent_category['price4']) && $curent_category['price4'] != ''){
                $price4 = $curent_category['price4'];
            }
            if(isset($this->data['price5']) && $this->data['price5'] != ''){
                $price5 = $this->data['price5'];
            }
            elseif(isset($curent_category['price5']) && $curent_category['price5'] != ''){
                $price5 = $curent_category['price5'];
            }
            if(isset($this->data['price6']) && $this->data['price6'] != ''){
                $price6 = $this->data['price6'];
            }
            elseif(isset($curent_category['price6']) && $curent_category['price6'] != ''){
                $price6 = $curent_category['price6'];
            }
            if(isset($this->data['price7']) && $this->data['price7'] != ''){
                $price6 = $this->data['price7'];
            }
            elseif(isset($curent_category['price7']) && $curent_category['price7'] != ''){
                $price6 = $curent_category['price7'];
            }
            if(isset($this->data['price8']) && $this->data['price8'] != ''){
                $price6 = $this->data['price8'];
            }
            elseif(isset($curent_category['price8']) && $curent_category['price8'] != ''){
                $price6 = $curent_category['price8'];
            }
            $params=array(
        ...
        public function FoodItemAdd()
        {
            ...
            $params=array(
                ...
                'free_item'=>isset($this->data['free_item'])?json_encode($this->data['free_item']):"[]",
                'exclude_item'=>isset($this->data['exclude_item'])?json_encode($this->data['exclude_item']):"[]",
                ...
                'meal_selection'=>isset($this->data['size1'])?json_encode($this->data['size1']):"",
            );
            ...
            if (isset($this->data['id']) && is_numeric($this->data['id'])){
                ...
                if ($res){
                    ...
                    /*Apply for all*/
                    if(isset($this->data['apply_all']) && $this->data['apply_all'] == 'yes'){
                        $new_params = array(
                            'free_item'=>$params['free_item'],
                            'exclude_item'=>$params['exclude_item'],
                            'multi_option'=>$params['multi_option'],
                            'multi_option_value'=>$params['multi_option_value'],
                            'require_addon'=>$params['require_addon'],
                            'meal_selection'=>$params['meal_selection'],
                            'addon_item'=>$params['addon_item']
                        );
                        if(isset($this->data['category']) && is_array($this->data['category'])){
                            $list_categories = '';
                            foreach($this->data['category'] as $apply_cat){
                                $stmt="
								SELECT * FROM
								{{item}}
								WHERE
								merchant_id='".Yii::app()->functions->getMerchantID()."'
								AND category LIKE '%\"".$apply_cat."\"%'
								ORDER BY item_id DESC";
                                $connection=Yii::app()->db;
                                $rows=$connection->createCommand($stmt)->queryAll();
                                if (is_array($rows) && count($rows)>=1){
                                    foreach ($rows as $val) {
                                        $res = $command->update('{{item}}' , $new_params ,'item_id=:item_id' , array(':item_id'=>addslashes($val['item_id'])));
                                    }
                                }
                            }
                        }
        ...
        public function filterCategoryItem(){
            $cat_id =$this->data['id'];
            $_SESSION['filter_item_category'] = $cat_id;
        }

        public function editPriceFoodItem(){
            $item_id = $this->data['id'];
            $new_price =$this->data['price'];
            $pos = $this->data['key'];
            $item = Yii::app()->functions->getFoodItem($item_id);
            $prices = json_decode($item['price'],true);
            $new_data = array();
            if(is_array($prices)){
                foreach($prices as $key=>$price){
                    if($key == $pos){
                        $new_data[$key] = $new_price;
                    }
                    else {
                        $new_data[$key] = $price;
                    }
            ...
            $params = array('price'=>json_encode($new_data));
            $this->updateData("{{item}}",$params,'item_id',$item_id);
        }
        ...
        public function FoodItemList()
        {
            ...
            if(isset($_SESSION['filter_item_category']) && $_SESSION['filter_item_category'] > 0){
                $stmt .= " AND category LIKE '%\"".$_SESSION['filter_item_category']."\"%'";
            }
            $stmt .= " ORDER BY item_id DESC";
            $connection=Yii::app()->db;
            $rows=$connection->createCommand($stmt)->queryAll();
            if (is_array($rows) && count($rows)>=1){
                foreach ($rows as $val) {
                    ...
                    if ( is_array($price) && count($price)>=1){
                        foreach ($price as $key_price=>$val_price) {
                            if (array_key_exists($key_price,(array)$size_list)){
                                $price_list .= '<p class="block_edit_item_price" id="block_edit_item_price_'.$val['item_id'].'_'.$key_price.'" style="margin:0px;display:none;"><input class="item_input_price" type="text" size="2" value="'.prettyFormat($val_price).'"><button class="uk-button save_edit_item_price" id="save_edit_item_price_'.$val['item_id'].'_'.$key_price.'" data-currency="'.getCurrencyCode().'" data-size="'.ucwords($size_list[$key_price]).'" type="button" data-id="'.$val['item_id'].'" data-key="'.$key_price.'">Save</button></p>';
                                $price_list.= '<span class="block_showing_price" id="block_showing_price_'.$val['item_id'].'_'.$key_price.'">'.getCurrencyCode().'<span class="item_ajax_price">'.prettyFormat($val_price)."</span> ".ucwords($size_list[$key_price]). " - <span data-id=\"".$val['item_id']."\" data-key=\"$key_price\" class=\"edit_item_price\" style=\"color:blue;cursor:pointer\">Edit</span></span><br/>";
        ...
        public function UpdateMerchant()
        {
            ...
            $params['authKey']=isset($this->data['authKey'])?$this->data['authKey']:'';
        ...
        public function merchantSettings()
        {
            ...
            if ( Yii::app()->functions->getOptionAdmin("website_time_picker_format") =="12"){
                ...
                if (is_array($this->data['stores_pickup_starts'])){
                    foreach ($this->data['stores_pickup_starts'] as $key=>$val) {
                        //dump($key."=>".$val);
                        $this->data['stores_pickup_starts'][$key]=timeFormat($val);
                    }
                }

                if (is_array($this->data['stores_pickup_ends'])){
                    foreach ($this->data['stores_pickup_ends'] as $key=>$val) {
                        //dump($key."=>".$val);
                        $this->data['stores_pickup_ends'][$key]=timeFormat($val);
                    }
                }
            }
            ...
            /*
	    	 * Added by CStar 25-15-2017
	    	 * */
            Yii::app()->functions->updateOption("stores_pickup_times_copy",
                isset($this->data['stores_pickup_times_copy'])?$this->data['stores_pickup_times_copy']:''
                ,$merchant_id);

            if (isset($this->data['stores_pickup_times_copy']) && $this->data['stores_pickup_times_copy'] == 'yes') {
                Yii::app()->functions->updateOption("stores_pickup_day",
                    isset($this->data['stores_open_day'])?json_encode($this->data['stores_open_day']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_starts",
                    isset($this->data['stores_open_starts'])?json_encode($this->data['stores_open_starts']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_ends",
                    isset($this->data['stores_open_ends'])?json_encode($this->data['stores_open_ends']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_pm_start",
                    isset($this->data['stores_open_pm_start'])?json_encode($this->data['stores_open_pm_start']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_pm_ends",
                    isset($this->data['stores_open_pm_ends'])?json_encode($this->data['stores_open_pm_ends']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_custom_text",
                    isset($this->data['stores_open_custom_text'])?json_encode($this->data['stores_open_custom_text']):''
                    ,$merchant_id);
            } else {
                Yii::app()->functions->updateOption("stores_pickup_day",
                ...
                Yii::app()->functions->updateOption("stores_pickup_starts",
                    isset($this->data['stores_pickup_starts'])?json_encode($this->data['stores_pickup_starts']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_ends",
                    isset($this->data['stores_pickup_ends'])?json_encode($this->data['stores_pickup_ends']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_pm_start",
                    isset($this->data['stores_pickup_pm_start'])?json_encode($this->data['stores_pickup_pm_start']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_pm_ends",
                    isset($this->data['stores_pickup_pm_ends'])?json_encode($this->data['stores_pickup_pm_ends']):''
                    ,$merchant_id);

                Yii::app()->functions->updateOption("stores_pickup_custom_text",
                    isset($this->data['stores_pickup_custom_text'])?json_encode($this->data['stores_pickup_custom_text']):''
                    ,$merchant_id);
            }
            /* End Update by CStar 28-12-2017*/
        ...
        public function specialInstrucitonsSelectionChange(){
            $_SESSION['kr_specialinstructions']['specialinstructions'] =$this->data['specialinstructions'];
        ...
        public function getOrderDeliveryTime(){
            ...
            $value = yii::app()->functions->getDeliveryTime($orderid);
            $dileryType = $value['trans_type'];
            $codeNumber = 1;
            $status = $value['status'];
            if($status=="CONFIRMED"){
                if($dileryType=="delivery"){
                    $result=$value['delivery_time'];
                }
                else{
                    $result='';
                }
        ...
        public function setDeliveryOptions()
        {
            ...
            if(!isset($_GET['iframe'])){
                ...
                $this->details=Yii::app()->createUrl('store/checkout?iframe');
            }
        }
            ...
            if (is_array($order_item) && count($order_item)>=1){
                ...
                switch ($this->data['payment_opt'])
                {
                    case "pyp":
                        ...
                        break;
                    case "sap":
                        $_SESSION['billing-different'] = $this->data['hdn-billing-different'];
                        $_SESSION['billing-address-1'] = $this->data['hdn-billing-address-1'];
                        $_SESSION['billing-address-2'] = $this->data['hdn-billing-address-2'];
                        $_SESSION['billing-city'] = $this->data['hdn-billing-city'];
                        $_SESSION['billing-postcode'] = $this->data['hdn-billing-postcode'];
                        $_SESSION['merchant-session'] = $this->data['hdn-merchant-session'];
                        $_SESSION['card-identifier'] = $this->data['hdn-card-identifier'];
                        break;
                    default:
                        break;
                }
                ...
                if ( Yii::app()->functions->code==1){
                    ...
                    //dump($raw);
                    /* if customer chooses an address from address book then we change the variables */
                    ...
                    if (is_array($raw) && count($raw)>=1){
                        $params=array(
                            ...
                            'special_instructions'=>isset($_SESSION['kr_specialinstructions']['specialinstructions'])?$_SESSION['kr_specialinstructions']['specialinstructions']:'',
                            'date_created'=>FunctionsV3::dateNow(),
                            'ip_address'=>$_SERVER['REMOTE_ADDR'],
                            ...
                            'deliverypostcode'=>isset($this->data['state'])?$this->data['state']:'',
                            //'authKey'=>isset($this->data['authKey'])?$this->data['authKey']:'',

                            'deliverycity'=>isset($this->data['city'])?$this->data['city']:''
...
//dump($params); die();
                        ...
                        if ($est_time = Yii::app()->functions->getOption("merchant_delivery_estimation",$_SESSION['kr_merchant_id'])) {
                        ...
                        if ( $this->insertData("{{order}}",$params)){
                            ...
                            foreach ($raw['item'] as $val) {
                                // Save
                                $food_item_info = Yii::app()->functions->getFoodItem($val['item_id']);
                                $list_free_items = json_decode($food_item_info['free_item'],true);
                                $list_exclude_items = json_decode($food_item_info['exclude_item'],true);
                                $addon_order = '';
                                if(isset($val['sub_item']) && is_array($val['sub_item'])){
                                    $addon_order_data = array();
                                    $number_addon = 0;
                                    foreach($val['sub_item'] as $key=>$val_sub){
                                        $addon_order_data[$key] = $val_sub;
                                        if(isset($list_free_items[$val_sub['category_id']]) && $list_free_items[$val_sub['category_id']] > 0 && $list_free_items[$val_sub['category_id']] != ''){
                                            if($number_addon < $list_free_items[$val_sub['category_id']]){
                                                if(isset($list_exclude_items[$val_sub['category_id']]) && is_array($list_exclude_items[$val_sub['category_id']]) && in_array($val_sub['addon_id'],$list_exclude_items[$val_sub['category_id']])){

                                                }
                                                else {
                                                    $number_addon++;
                                                    $addon_order_data[$key]['addon_price'] = false;
                                                }
                                            }
                                            else {
                                            }
                                    ...
                                    $addon_order = json_encode($addon_order_data);
                                }
                            ...
                            switch ($this->data['payment_opt'])
                            {
                                ...
                                case "sap":
                                    ...
                                    $this->details['new_merchant_session'] = $request['body']['merchantSessionKey'];
                                    $this->msg=Yii::t("default","Please wait...");
                                    break;
                                default:
                            ...
                            $this->details = array_merge($this->details, array(
                                'order_id'=>$order_id,
                        ...
                        if ($res=$db_ext->rst($stmt)){
                            foreach ($res as $val) {
                                $data[]=array(
                                    ...
                                    $val['street']." ".$val['city']." ".$val['state']." ".$val['country_code']." ".$val['post_code']." ".$val['authKey'],
        ...
        public function adminSettings()
        {
            ...
            Yii::app()->functions->updateOptionAdmin("home_search_text_mobile",
                isset($this->data['home_search_text_mobile'])?$this->data['home_search_text_mobile']:'');

            Yii::app()->functions->updateOptionAdmin("home_search_subtext_mobile",
                isset($this->data['home_search_subtext_mobile'])?$this->data['home_search_subtext_mobile']:'');
...
//                    $this->msg=t("Sorry you cannot apply voucher, exising discount is already applied in your cart");
//                    return ;
//                }
//            }
            ...
            if (isset($this->data['voucher_code'])){
                if ( $res=Yii::app()->functions->getVoucherCodeNew($this->data['voucher_code'],$this->data['merchant_id']) ){
                    ...
                    /*check if voucher code can be used only once*/ /* Added By Pei*/
                    if ( $res['voucher_over']){
                        if ( $res['voucher_over'] >= $this->data['subtotal']){
                            $msg = Yii::app()->functions->jsLanguageAdmin();
                            $msg = str_replace("Voucher Over", "£" . normalPrettyPrice($res['voucher_over']), $msg['trans_53']);
                            $this->code = 0;
                            $this->msg = $msg;
                            return ;
                        }
        ...
        public function themeSettings()
        {
            ...
            Yii::app()->functions->updateOptionAdmin("disabled_featured_merchant_mobile",
                isset($this->data['disabled_featured_merchant_mobile'])?$this->data['disabled_featured_merchant_mobile']:'');
            ...
            Yii::app()->functions->updateOptionAdmin("theme_hide_footer_section1_mobile",
                isset($this->data['theme_hide_footer_section1_mobile'])?$this->data['theme_hide_footer_section1_mobile']:'');
            ...
            Yii::app()->functions->updateOptionAdmin("theme_hide_footer_section2_mobile",
                isset($this->data['theme_hide_footer_section2_mobile'])?$this->data['theme_hide_footer_section2_mobile']:'');
        ...
        public function markAsViewReceipts()
        {
            if (isset($this->data['backend'])){
                if ( $this->data['currentController']=="admin"){
                    $params=array( 'admin_viewed'=>1 );
                ...
                $orderIds = explode(",", $this->data['ids']);
                if (is_array($orderIds)) {
                    foreach ($orderIds as $order_id) {
                        $this->updateData("{{order}}",$params,'order_id', $order_id);
                    }
                } else {
                    $this->updateData("{{order}}",$params,'order_id', $orderIds);
                }
